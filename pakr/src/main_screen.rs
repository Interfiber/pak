use gtk::{self, Application, Window, Button, Orientation, Label};
use gtk::prelude::*;
use std::env;
use nfd::Response;
use crate::pak_cli;
    ...
    new_project_button.connect_clicked(move |_| {
        ...
        match result {
            Response::Okay(file_path) => {
                ...
                println!("{:?}", std::process::Command::new(current_path.into_os_string().into_string().unwrap()).arg("load_project").arg(path.to_string()).output().expect("Failed to open project"));
            },
